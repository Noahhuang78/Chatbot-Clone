...
with open(f'perf_time.jsonl', 'a', encoding='utf-8') as f:
    json.dump({
        'name': 'google/embeddinggemma-300m', 'perf_time': elapsed_time}, f, ensure_ascii=False)

with open(f'similarities_dist.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False)

df = pd.DataFrame(results)
print(f"My DF:\n {df}")
avg_simlarity = df.iloc[:, 0].mean()

print(f'Average similarity distance: {avg_simlarity}')
#average for gemma is 0.490437
