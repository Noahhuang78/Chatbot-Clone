...
# --------------------------------
def is_caps_header(text):
    if len(text) < 8:
        return False
    words = [w for w in text.split() if w.isalpha()]
    caps_ratio = sum(w.isupper() for w in words) / max(1, len(words))
    return caps_ratio > 0.7 and len(words) >= 3
        ...
        for block in blocks:
            ...
            for line in block["lines"]:
                for span in line["spans"]:
                    ...
                    # Accept as header if:
                    # - Large font and numbered
                    # - Large font and phrase (>3 words)
                    # - Sufficiently capitalized phrase
                    if (
                        size >= 12 and (pattern_numbered or len(text.split()) > 3)
                        or is_caps_header(text)
                    ):
                        # Assign warning level for caps phrases
                        if is_caps_header(text):
                            level = "warning"
                        elif pattern_numbered:
...
# --------------------------------
# Save Individual PDF Pages (Optional)
# --------------------------------
